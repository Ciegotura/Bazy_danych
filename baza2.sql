...
SELECT pracownicy.imie, pracownicy.nazwisko, godziny.liczba_godzin -140 AS Nadgodziny 
...
AND ksiegowosc.godziny.id_godziny = ksiegowosc.wynagrodzenie.id_godziny
AND ksiegowosc.premia.id_premii = ksiegowosc.wynagrodzenie.id_premii
AND ksiegowosc.godziny.liczba_godzin > 120
AND ksiegowosc.premia.kwota = 0;

--i) Uszereguj pracowników wed³ug pensji.

SELECT pracownicy.imie, pracownicy.nazwisko, ksiegowosc.pensja.kwota 
...
SELECT pracownicy.imie, pracownicy.nazwisko, ksiegowosc.pensja.kwota + ksiegowosc.premia.kwota AS Dochod 
FROM ksiegowosc.pracownicy, ksiegowosc.pensja, ksiegowosc.wynagrodzenie, ksiegowosc.premia
WHERE ksiegowosc.pracownicy.id_pracownika = ksiegowosc.wynagrodzenie.id_pracownika 
AND ksiegowosc.pensja.id_pensji = ksiegowosc.wynagrodzenie.id_pensji
AND ksiegowosc.premia.id_premii = ksiegowosc.wynagrodzenie.id_premii
...
SELECT AVG(pensja.kwota) AS Srednia_placa, MAX(pensja.kwota) AS Maksymalna_placa, MIN(pensja.kwota) AS Minimalna_placa 
...
SELECT * FROM ksiegowosc.pracownicy; 
--modyfikacja wielkosci zmiennej varchar
...
UPDATE ksiegowosc.pracownicy SET telefon = CONCAT(SUBSTRING(telefon, 1, 3), 
...
SELECT UPPER(ksiegowosc.pracownicy.nazwisko) AS najdluzsze_nazwisko
...
SELECT ksiegowosc.pracownicy.imie, ksiegowosc.pracownicy.nazwisko, HASHBYTES('MD5', CAST(ksiegowosc.pensja.kwota AS VARCHAR(10))) AS kwota_zakodowana
FROM ksiegowosc.pracownicy, ksiegowosc.pensja, ksiegowosc.wynagrodzenie
WHERE ksiegowosc.pracownicy.id_pracownika = ksiegowosc.wynagrodzenie.id_pracownika 
AND ksiegowosc.pensja.id_pensji = ksiegowosc.wynagrodzenie.id_pensji;
...
SELECT CONCAT('Pracownik ', p.imie, ' ', p.nazwisko, ', w dniu ', CONVERT(VARCHAR(10), wy.data, 104),
' otrzyma³ pensjê ca³kowit¹ na kwotê ', CONVERT(VARCHAR(10), pe.kwota), ' z³, gdzie \n wynagrodzenie zasadnicze wynosi³o: ',
CONVERT(VARCHAR(10), pe.kwota),' z³, premia: ',CONVERT(VARCHAR(10), pr.kwota),' z³, nadgodziny: ',
CONVERT(VARCHAR(10),(g.liczba_godzin-140)*25),'z³') AS raport
FROM ksiegowosc.pracownicy p
JOIN ksiegowosc.wynagrodzenie wy ON p.id_pracownika = wy.id_pracownika
JOIN ksiegowosc.pensja pe ON wy.id_wynagrodzenia = wy.id_wynagrodzenia
JOIN ksiegowosc.premia pr ON wy.id_premii = pr.id_premii
JOIN ksiegowosc.godziny g ON  wy.id_godziny = g.id_godziny
WHERE p.imie = 'Tomasz';
...
CONVERT(VARCHAR(10), pe.kwota),' z³, premia: ',CONVERT(VARCHAR(10), pr.kwota),' z³, nadgodziny: ',
CONVERT(VARCHAR(10),(g.liczba_godzin-140)*25),'z³') AS raport
FROM ksiegowosc.pracownicy p, ksiegowosc.pensja pe, ksiegowosc.wynagrodzenie wy, ksiegowosc.premia pr, ksiegowosc.godziny g
WHERE p.id_pracownika = wy.id_pracownika 
AND pe.id_pensji = wy.id_pensji
AND pr.id_premii = wy.id_premii
AND g.id_godziny = wy.id_wynagrodzenia
AND p.imie='Tomasz';
